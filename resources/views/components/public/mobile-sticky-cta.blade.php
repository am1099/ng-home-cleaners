...
<div class="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface-page/95 p-3 backdrop-blur-sm lg:hidden" style="padding-bottom: max(0.75rem, env(safe-area-inset-bottom));">
    <div class="mx-auto flex max-w-lg gap-2">
        ...
        @if ($showEstimate)
            <x-public.estimate-cta label="Get estimate" class="min-w-0 flex-1 justify-center" size="sm" location="mobile_sticky" />
        @endif
        <x-public.whatsapp-cta label="WhatsApp" class="min-w-0 flex-1 justify-center" size="sm" />
    </div>
</div>
