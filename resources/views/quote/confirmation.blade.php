<x-layouts.quote>
    <x-public.section>
        <x-public.container narrow>
            ...
            @php
                $whatsappHref = $whatsappUrl ?: \App\Models\SiteSetting::instance()->whatsappLink();
            @endphp
            ...
            <div class="mt-10 rounded-[var(--radius-card)] border border-border bg-surface-sunken/40 p-6">
                ...
                <p class="mt-2 text-sm text-ink-muted">
                    Your email request is saved. You can still continue on WhatsApp to send a short walkthrough video of the property (kitchen, bathrooms, main rooms). That is the best way for us to give you an accurate fixed quote. Long videos should be sent on WhatsApp only — they are not uploaded to our website.
                </p>
                <div class="mt-5 flex flex-wrap gap-3">
                    @if ($whatsappHref)
                        <x-public.button href="{{ $whatsappHref }}" variant="whatsapp" target="_blank" rel="noopener noreferrer">
            ...
            <div class="mt-10 rounded-[var(--radius-card)] border border-border bg-surface-card p-6">
                <h2 class="ng-display text-xl">Know someone moving?</h2>
                <p class="mt-2 text-sm text-ink-muted">
                    Share this estimate link with a friend or landlord who needs a clean before or after a move. No booking is made until they confirm.
                </p>
                <p class="mt-4">
                    <a href="{{ route('quote') }}" class="ng-link">{{ route('quote') }}</a>
                </p>
            </div>
        </x-public.container>
    </x-public.section>
