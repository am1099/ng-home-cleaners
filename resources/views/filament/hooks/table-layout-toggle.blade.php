{{-- Cards / Table layout toggle — wired to HasToggleableRecordLayout --}}
<div
...
>
    <button
        type="button"
        wire:click="setTableLayoutView('grid')"
        @class([
            'inline-flex items-center justify-center gap-1 rounded-md px-2.5 py-1.5 text-sm font-semibold transition',
            'bg-white text-gray-950 shadow-sm dark:bg-white/10 dark:text-white' => $isGrid,
            'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' => ! $isGrid,
        ])
    ...
    <button
        type="button"
        wire:click="setTableLayoutView('list')"
        @class([
            'inline-flex items-center justify-center gap-1 rounded-md px-2.5 py-1.5 text-sm font-semibold transition',
            'bg-white text-gray-950 shadow-sm dark:bg-white/10 dark:text-white' => $isList,
            'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' => ! $isList,
        ])
        aria-label="Table view"
        aria-pressed="{{ $isList ? 'true' : 'false' }}"
    >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="h-4 w-4" aria-hidden="true">
