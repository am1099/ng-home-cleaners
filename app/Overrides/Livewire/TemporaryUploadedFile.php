...
class TemporaryUploadedFile extends UploadedFile
{
    ...
    public function __construct($path, $disk)
    {
        ...
        // Do not use tmpfile() — on some Windows setups it returns false
        // (locked system TEMP / antivirus), which crashes stream_get_meta_data().
        // Symfony UploadedFile only needs a real pathname; bytes are read from the Livewire disk.
        parent::__construct($this->resolveLocalPathname(), $this->path, null, null, true);
    ...
    /**
     * Pathname for the Symfony UploadedFile parent — never uses tmpfile().
     */
    protected function resolveLocalPathname(): string
    {
        $stored = $this->storage->path($this->path);
        ...
        $placeholder = tempnam($dir, 'lw_');

        if ($placeholder === false) {
            $placeholder = $dir.DIRECTORY_SEPARATOR.'lw_'.uniqid('', true);
            touch($placeholder);
        }
